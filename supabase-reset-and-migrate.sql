-- ============================================
-- SUPABASE DATABASE RESET & MIGRATION SCRIPT
-- ============================================
-- This script will:
-- 1. Drop all existing tables and types
-- 2. Apply the new schema for Dumpster CRM
-- ============================================
...
-- Drop old tables from previous CRM (add any you might have)
DROP TABLE IF EXISTS cities CASCADE;
DROP TABLE IF EXISTS content CASCADE;
DROP TABLE IF EXISTS pages CASCADE;
DROP TABLE IF EXISTS jobs CASCADE;

-- Step 2: Drop custom types (if they exist)
...
DROP TRIGGER IF EXISTS update_geo_locations_updated_at ON geo_locations;
DROP TRIGGER IF EXISTS update_research_jobs_updated_at ON research_jobs;
DROP TRIGGER IF EXISTS update_wordpress_pages_updated_at ON wordpress_pages;
...
SELECT 'Database reset and migration complete!' as status,
       (SELECT COUNT(*) FROM geo_locations) as cities_loaded,
       'Schema version: 1.0.0' as version;
